...
function TrailCalendar(props) {
  ...
  const handleEvents = 
    (slotInfo) => {
      ...
      let d1 = new Date(slotInfo.start);
      let d2 = new Date(slotInfo.end);
      if (slotInfo.start >= today) {
        console.log(selectedSlots);
        const found = selectedSlots.some((el) => {
          console.log(el);
          return (
            el.start.getDate() === d1.getDate() &&
            el.end.getDate() === d2.getDate() &&
            el.title ===
        ...
        if (!found) {
          setSelectedSlots((prev) => [
            ...prev,
            {
              start: d1,
                ...
                title: convertFrom24To12Format(String(slotInfo.start).slice(16, 21)) +
                "-" +
                convertFrom24To12Format(String(slotInfo.end).slice(16, 21))
            },
          ]);
          ...
          selectedSlots.push({
            ...
            title:
              convertFrom24To12Format(String(slotInfo.start).slice(16, 21)) +
              "-" +
              convertFrom24To12Format(String(slotInfo.end).slice(16, 21)),
          });
          console.log(selectedSlots);
